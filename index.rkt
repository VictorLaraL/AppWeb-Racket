    ...
    (response/xexpr
     `(html (head (title "Racket Web App")
            ...
            (body (h1 "Merge Sort"), (render-result-MD listElements)
                  (p "Primer dato moda, segundo dato cantidad de veces que se repite")
                (form ((action , (embed/url insert-post-handler)))
                 (input ((name "elemento")))
                 (input ((type "submit")))
                 ),
                (render-list listElements)
                  (h2 "Algoritmo")
                  (p "(define (calcularmoda lista hashf)")
                  (p "(hash-update! hashf")
                  (p "(car lista)")
                  (p "(lambda (frec) (add1 frec)) 0)")
                
                  (p "(cond [(not (null? (cdr lista))) (calcularmoda (cdr lista) hashf)]")
                  (p "[else (for/fold ([mod null]")
                  (p "[mfrec 0])")
                  (p "([(val cant) (in-hash hashf)])")
                  (p "(cond [(> cant mfrec) (values val cant)]")
                  (p "[else (values mod mfrec)]))]))")

                  (p "(define (moda lista)")
                  (p "(let ([frecuencias (make-hash)])")
                  (p "(let-values ([(x y) (calcularmoda lista frecuencias)])")
                  (p "(list x y))))")
                  (p (a ([href ,(embed/url home)])
                        "Home"))))))
    (define (insert-post-handler request)
